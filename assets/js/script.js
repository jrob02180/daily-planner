...
// If timeblock is < current time, display gray
// If timeblock is === current time, display red
// If timeblock is > current time, display green
...
// To have it display the string it was entered as

var todaysDate = dayjs().format('dddd, MMMM D');
var saveButton = $('.btn');
var currentHour = dayjs().hour();
var todayEl = $('#currentDay');
todayEl.text(todaysDate);

// Loops through schedule, changes color based on current vs previous vs future time
for (let i = 9; i < 18; i++) {
  var hour = $('#hour-' + i)
  if (i < currentHour) {
    hour.addClass('past')
  ...
  // Saves text from input area to local storage
